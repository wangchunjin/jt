...
var jbImagesDialog = {
	...
	inProgress : function() {
		...
		this.timeoutStore = window.setTimeout(function(){
			document.getElementById("upload_additional_info").innerHTML = 'This is taking longer than usual.' + '<br />' + 'An error may have occurred.' + '<br />';
	...
	uploadFinish : function(result) { 
		if (result[0].resultCode == 'failed')
		{
			...
			document.getElementById("upload_infobar").innerHTML = result[0].result;
	...
	insert :function(fileType,filePath,fileName){
		var w = this.getWin();
			tinymce = w.tinymce;
		    if(fileType=='IMG'){
			    tinymce.EditorManager.activeEditor.insertContent('<img src="' + filePath +'">');
			}else{
			    tinymce.EditorManager.activeEditor.insertContent("<a href='" + filePath +"'  style=' border-bottom:1px blue solid;' >"+fileName+"</a>");
			}
	},
	getWin : function() {
