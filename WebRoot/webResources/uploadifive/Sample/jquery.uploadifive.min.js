			...
			return this
					.each(function() {
						...
						d.addQueueItem = function(k) {
							...
							if (f.notFileType) {
								//if (b.isArray(f.notFileType)) {
								if(true){
									var j = false; 
									var   accept = '\\.' + f.notFileType
	                                 .replace( /,/g, '$|\\.' )
	                                  .replace( /\*/g, '.*' ) + '$';
									accept = new RegExp( accept, 'i' );
								    rExt = /\.\w+$/; 
									/*for (var l = 0; l < f.notFileType.length; l++) {
										if (k.type.indexOf(f.notFileType[l]) > -1) {
											j = true;
										}
									}*/ 
								    if(!rExt.exec( k.name )){
								    	d.error("FORBIDDEN_FILE_TYPE", k);
								    }else if (rExt.exec( k.name ) &&  accept.test( k.name )) {
										d.error("FORBIDDEN_FILE_TYPE", k);
									}
								} else {
									if (k.type.indexOf(f.notFileType) < 0) {
										d.error("FORBIDDEN_FILE_TYPE", k);
									}
								}
							}
						...
						d.error = function(l, j, k) {
							if (b.inArray("onError", f.overrideEvents) < 0) {
								...
								case "FORBIDDEN_FILE_TYPE":
									errorMsg = "<font style='color:red'>文件格式不允许上传</font>";
									break;
