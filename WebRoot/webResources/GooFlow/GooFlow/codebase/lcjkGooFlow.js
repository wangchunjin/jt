...
GooFlow.prototype={
	...
	initDraw:function(id,width,height){
		...
		$(this.$draw).delegate(tmpClk,"click",{inthis:this},function(e){
			parent.document.all('ThreeFrame').src = property.projectPath+"/common/label.jsp?Module=WF_LCJK_LINE&Where="+processdefid+";"+this.id+";"+processinstid;
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".GooFlow_item","click",{inthis:this},function(e){
			...
			if(e.data.inthis.$nodeData[this.id].type=="start"||e.data.inthis.$nodeData[this.id].type=="end"){
				parent.document.all('ThreeFrame').src = property.projectPath+"/common/label.jsp?Module=WF_LCJK&Where="+processdefid+";"+processinstid+";"+e.data.inthis.$nodeData[this.id].ACTIVITYINSTID;
			}else{
				parent.document.all('ThreeFrame').src = property.projectPath+"/common/label.jsp?Module=WF_LCJK_NODE&Where="+processdefid+";"+this.id+";"+processinstid+";"+e.data.inthis.$nodeData[this.id].ACTIVITYINSTID;	
			}
			e.data.inthis.focusItem(this.id,true);
