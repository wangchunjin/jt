...
//初始化区域图的对象
	...
	this.$bgDiv.append("<div class='GooFlow_work' style='margin: 0px 3px 3px 0px;width:100%;height:98%;"+(property.haveHead? "":"margin-top:3px")+"'></div>");
	...
	if(this.$editable){
	  this.$workArea.on("click",{inthis:this},function(e){
		...
		var nodeName = e.data.inthis.$nodeRemark[type];
		var flag = "0";
		if(e.data.inthis.$nowType == "start" || e.data.inthis.$nowType == "end"){
			for(var i in e.data.inthis.$nodeData){
				if(e.data.inthis.$nodeData[i].type == "start"){
					flag ="1";
					break;
				}
				
				if(e.data.inthis.$nodeData[i].type == "end"){
					flag ="1";
					break;
				}
			}
		...
		if(flag =="0"){
			e.data.inthis.addNode(e.data.inthis.$id+"_node_"+e.data.inthis.$max+"_"+getRandomString(10),{name:nodeName,left:X,top:Y,type:e.data.inthis.$nowType});
			e.data.inthis.$max++;
		}else{
...
GooFlow.prototype={
	...
	initDraw:function(id,width,height){
		...
		$(this.$draw).delegate(tmpClk,"click",{inthis:this},function(e){
			...
			parent.document.all('ThreeFrame').src = property.projectPath+"/common/label.jsp?Module=WF_LCTZ_LINE&Where="+processdefid+";"+this.id;
			e.data.inthis.focusItem(this.id,true);
		});
		$(this.$draw).delegate(tmpClk,"dblclick",{inthis:this},function(e){
			//alert("----2222-----");
			//popWin.showWin("700","300","编辑条件",property.projectPath+"/web/WF_ENT/LCDY_ENT/lineCondition.jsp?lineId="+this.id+"&processdefid="+processdefid);
	...
	addNode:function(id,json){
		...
		if(json.type.indexOf(" round")<0){
			...
			//alert(id+"-----"+json.PROCESSINSTID+"----"+json.ACTIVITYINSTID);
			var titleDiv = "";
			if(isNull(json.nodeOperaLog)){
				var operaLogData = json.nodeOperaLog;
				for(var i=0;i<operaLogData.length;i++){
					titleDiv += "("+(operaLogData.length-i)+")"+operaLogData[i].OPERATOR_NAME+"--"+operaLogData[i].OPERATEACTION;
					if(isStringNull(operaLogData[i].TOUSER_NAME)){
						titleDiv += "-->"+operaLogData[i].TOUSER_NAME
					}
					titleDiv +="("+operaLogData[i].CURRENTDATE+")";
					titleDiv += "\n";
				}
			}
			if(json.CURRENTSTATE == "INITIATED"){//初始化-灰
				this.$nodeDom[id]=$("<div class='GooFlow_item"+mark+"' title = '"+titleDiv+"' id='"+id+"' style='background:#D3D3D3;top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='1' style='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'><tr><td class='ico'><b class='ico_"+json.type+"'></b></td><td>"+json.name+"</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div></div></div>");
			}else if(json.CURRENTSTATE == "RUNNING"){//运行-红
				this.$nodeDom[id]=$("<div class='GooFlow_item"+mark+"' title = '"+titleDiv+"' id='"+id+"' style='background:red;top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='1' style='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'><tr><td class='ico'><b class='ico_"+json.type+"'></b></td><td>"+json.name+"</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div></div></div>");
			}else if(json.CURRENTSTATE == "COMPLETED"){//完成-绿
				this.$nodeDom[id]=$("<div class='GooFlow_item"+mark+"' title = '"+titleDiv+"' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='1' style='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'><tr><td class='ico'><b class='ico_"+json.type+"'></b></td><td>"+json.name+"</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div></div></div>");
			}else if(json.CURRENTSTATE == "SUSPENDED"){//挂起-蓝
				this.$nodeDom[id]=$("<div class='GooFlow_item"+mark+"' title = '"+titleDiv+"' id='"+id+"' style='background:blue;top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='1' style='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'><tr><td class='ico'><b class='ico_"+json.type+"'></b></td><td>"+json.name+"</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div></div></div>");
			}else if(json.CURRENTSTATE == "WAITING"){//等待-黄
				this.$nodeDom[id]=$("<div class='GooFlow_item"+mark+"' title = '"+titleDiv+"' id='"+id+"' style='background:yellow;top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='1' style='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'><tr><td class='ico'><b class='ico_"+json.type+"'></b></td><td>"+json.name+"</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div></div></div>");
			}else if(json.CURRENTSTATE == "TERMINATED"){//终止
				this.$nodeDom[id]=$("<div class='GooFlow_item"+mark+"' title = '"+titleDiv+"' id='"+id+"' style='background:black;top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='1' style='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'><tr><td class='ico'><b class='ico_"+json.type+"'></b></td><td>"+json.name+"</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div></div></div>");
			}else if(json.CURRENTSTATE == "ROLLBACK"){//回退
				this.$nodeDom[id]=$("<div class='GooFlow_item"+mark+"' title = '"+titleDiv+"' id='"+id+"' style='background:green;top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='1' style='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'><tr><td class='ico'><b class='ico_"+json.type+"'></b></td><td>"+json.name+"</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div></div></div>");
			}else if(json.CURRENTSTATE == "SKIP"){//跳过
				this.$nodeDom[id]=$("<div class='GooFlow_item"+mark+"' title = '"+titleDiv+"' id='"+id+"' style='background:#D3D3D3;top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='1' style='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'><tr><td class='ico'><b class='ico_"+json.type+"'></b></td><td>"+json.name+"</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div></div></div>");
			}else{
				this.$nodeDom[id]=$("<div class='GooFlow_item"+mark+"' title = '"+titleDiv+"' id='"+id+"' style='background:#ffffff;top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='1' style='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'><tr><td class='ico'><b class='ico_"+json.type+"'></b></td><td>"+json.name+"</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div></div></div>");
			}
			if(json.type.indexOf(" mix")>-1)	this.$nodeDom[id].addClass("item_mix");
		}
		else{
			json.width=24;json.height=24;
			this.$nodeDom[id]=$("<div class='GooFlow_item item_round"+mark+"' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='0'><tr><td class='ico'><b class='ico_"+json.type+"'></b></td></tr></table><div  style='display:none'></div><div class='span'>"+json.name+"</div></div>");
		}
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".GooFlow_item","click",{inthis:this},function(e){
			if(e.data.inthis.$nodeData[this.id].type=="start"||e.data.inthis.$nodeData[this.id].type=="end"){
				parent.document.all('ThreeFrame').src = property.projectPath+"/common/label.jsp?Module=WF_LCTZ_LC&Where="+processdefid+";"+this.id;
			}else{
				parent.document.all('ThreeFrame').src = property.projectPath+"/common/label.jsp?Module=WF_LCTZ_NODE&Where="+processdefid+";"+this.id;	
			}
			e.data.inthis.focusItem(this.id,true);
		...
		this.$workArea.delegate(".GooFlow_item","mouseup",{inthis:this},function(e){
			...
			if(lineStart)	This.addLine(This.$id+"_line_"+This.$max+"_"+getRandomString(10),{from:lineStart.id,to:this.id,name:""});
			This.$max++;
		});
		//绑定双击编辑事件
		...
		this.$workArea.delegate(".ico + td","dblclick",{inthis:this},function(e){
			...
			//alert("----1111-747行----节点类型："+nodeType);
			if(nodeType == "task"){
			//	popWin.showWin("900","600","编辑人工活动",property.projectPath+"/web/WF_ENT/LCDY_ENT/nodeCondition.jsp?nodeId="+id+"&processdefid="+processdefid);
			}
...
///////////以下为有关画线的方法
	...
	addArea:function(id,json){
		...
		+"<label>"+json.name+"</label><b></b><div><div class='rs_bottom'></div><div class='rs_right'></div></div></div>");
...
function getRandomString(len){
	var len = len || 32;
	var $chars = "ABCDEFGHI1234567890JKLMNOPQRSTU1234567890VWXYZabcdefghijklmnopqrstuvwxyz1234567890";
...
function showTitleDiv(projectPath,processinstid,activityinstid){
	//alert(processinstid+"--------"+activityinstid);
	//alert(projectPath+"/wf_ent/wf_comm/showNodeOperateLog.action");
	//var res = IntoActionByUrl(projectPath+"/wf_ent/wf_comm/showNodeOperateLog.action","processinstid="+processinstid+"&activityinstid="+activityinstid);
