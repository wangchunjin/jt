...
GooFlow.prototype={
	...
	initDraw:function(id,width,height){
		...
		$(this.$draw).delegate(tmpClk,"click",{inthis:this},function(e){
			...
			parent.document.all('ThreeFrame').src = property.projectPath+"/common/label.jsp?Module=WF_LCTZ_LINE_ENT&Where="+processdefid+";"+this.id;
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".GooFlow_item","click",{inthis:this},function(e){
			if(e.data.inthis.$nodeData[this.id].type=="start"||e.data.inthis.$nodeData[this.id].type=="end"){
				parent.document.all('ThreeFrame').src = property.projectPath+"/common/label.jsp?Module=WF_LCTZ_LC_ENT&Where="+processdefid+";"+this.id;
			}else{
				parent.document.all('ThreeFrame').src = property.projectPath+"/common/label.jsp?Module=WF_LCTZ_NODE_ENT&Where="+processdefid+";"+this.id;	
			}
			e.data.inthis.focusItem(this.id,true);
