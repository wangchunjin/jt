...
GooFlow.prototype={
	...
	initDraw:function(id,width,height){
		...
		$(this.$draw).delegate(tmpClk,"click",{inthis:this},function(e){
			parent.document.all('ThreeFrame').src = property.projectPath+"/common/label.jsp?Module=WF_LCJK_LINE_ENT&Where="+processdefid+";"+this.id+";"+processinstid;
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".GooFlow_item","click",{inthis:this},function(e){
			//alert(e.data.inthis.$nodeData[this.id].ACTIVITYINSTID+"-----"+e.data.inthis.$nodeData[this.id].PROCESSINSTID);
			if(e.data.inthis.$nodeData[this.id].type=="start"||e.data.inthis.$nodeData[this.id].type=="end"){
				parent.document.all('ThreeFrame').src = property.projectPath+"/common/label.jsp?Module=WF_LCJK_ENT&Where="+processdefid+";"+processinstid+";"+e.data.inthis.$nodeData[this.id].ACTIVITYINSTID;
			}else{
				parent.document.all('ThreeFrame').src = property.projectPath+"/common/label.jsp?Module=WF_LCJK_NODE_ENT&Where="+processdefid+";"+this.id+";"+processinstid+";"+e.data.inthis.$nodeData[this.id].ACTIVITYINSTID;	
			}
			e.data.inthis.focusItem(this.id,true);
