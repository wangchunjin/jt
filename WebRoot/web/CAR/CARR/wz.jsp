...
<html>
 ...
 <script type="text/javascript">
	     function AddRecord(){
	        ...
	        var res = $('#addForm').SubmitForm("${ctx}/car/tcar/wz.action");
...
// 						var result = GetXmlBySql("SELECT num FROM T_pinpaituijian WHERE isdel='0' ORDER BY num ASC limit 0,1");
	    		...
	    		//去最后一条数据的num
	    		 ...
	    		 var result = GetXmlBySql("SELECT orderflag FROM T_car WHERE isdel='0' and type='0' ORDER BY orderflag ASC limit "+num1+",1");
...
// 	    		 return;
			...
			if(num/1<=srow){ //输入位置数<=总条数
				...
				if(num/1==1){
					...
					var result2 = GetXmlBySql("SELECT orderflag FROM T_car WHERE isdel='0' and type='0' ORDER BY orderflag ASC limit "+num2+",1");
				...
				//获取选中车型的orderflag
	    		 var rr = GetXmlBySql("SELECT orderflag FROM T_car WHERE id="+${param.cid });
	    		 ...
	    		 //获取输入位置车型的orderflag
	    		  var rr1 = GetXmlBySql("SELECT orderflag FROM T_car WHERE isdel='0' and type='0' ORDER BY orderflag ASC limit "+numoo1+",1");
	    		 ...
	    		 if(oo1>oo){
		    		...
		    		//上一个位置数的num
		    		 var result1 = GetXmlBySql("SELECT orderflag FROM T_car WHERE isdel='0' and type='0' ORDER BY orderflag ASC limit "+num1+",1");
		    		//下一个位置数的num
		    		 var result2 = GetXmlBySql("SELECT orderflag FROM T_car WHERE isdel='0' and type='0' ORDER BY orderflag ASC limit "+num2+",1");
	     ...
	     $(function(){
	    	 //所有条数
	    	 var result = GetXmlBySql("SELECT count(id)num FROM T_car WHERE isdel='0' and type='0'");	    	 
   ...
   <form method="post" action="" id="addForm" name="addForm">
		<table style="margin-top: 15px;width: 96%;" >
			...
			<tr>
				...
				<td width="20%">
				<input class="form_text" type="hidden"  name="car.id" id="id"  value="${param.cid }" >
			...
			<tr >
				<td>
					...
					<!-- 排序数敬上 -->
					<input class="form_text" type="hidden" name="car.orderflag"  id="mingsuan"  value="" >
				</td>
