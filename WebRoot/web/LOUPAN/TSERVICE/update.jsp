...
// 	        	}
// 	        }
		var res = $('#addForm').SubmitForm("${ctx}/tservice/ttservice/update.action");
   ...
   <form method="post" action="" id="addForm" name="addForm">
		<table style="margin-top: 15px;width: 96%;" >
			<tr>
				...
				<td width="30%">
				<input type="hidden" class="form_text"  name="tservice.id" id="id"  value="${tservice[0].id }">
				<div class="lf salebd"><a href="#"  id="preview"><img id="imghead"  src="${ctx}${tservice[0].pic}" width="NAN" height="100" /></a></div><br>
			...
			<tr>
				...
				<td width="20%">
				<epmis:select  id="tservice.status" define="select id,name from t_service_type where isdel='0'  " value="${tservice[0].status}" attr="style='width: 100%;'"  ></epmis:select>
<!-- 					<select class="form_text" name="tservice.status" id="status"  style="width: 100%;"> -->
<!-- 						<option value="1" <c:if test="${tservice[0].status==1 }">selected="selected"</c:if>>服务理念</option> -->
<!-- 						<option value="2" <c:if test="${tservice[0].status==2 }">selected="selected"</c:if>>服务文化</option>	 -->
<!-- 						<option value="3" <c:if test="${tservice[0].status==3 }">selected="selected"</c:if>>专车看房</option>	 -->
<!-- 						<option value="4" <c:if test="${tservice[0].status==4 }">selected="selected"</c:if>>金牌顾问</option>	 -->
<!-- 						<option value="5" <c:if test="${tservice[0].status==5 }">selected="selected"</c:if>>楼盘分析</option>	 -->
<!-- 					</select> -->
				</td>
			</tr>
			<tr>
				...
				<td width="20%">
					...
					<select class="form_text" name="tservice.type" id="type" onchange="check()" style="width: 100%;">
						...
						<option value="2" <c:if test="${tservice[0].type==2 }">selected="selected"</c:if>>服务链接</option>
						<option value="1" <c:if test="${tservice[0].type==1 }">selected="selected"</c:if>>服务文本</option>
			...
			<tr id="ttt">
				...
				<td width="20%"><input class="form_text"  name="tservice.url" id="url"  value="${tservice[0].url }"></td>
				</tr>
