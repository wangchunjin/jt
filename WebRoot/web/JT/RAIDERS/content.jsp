...
function look(obj){
    createSimpleWindow("lookWindow","产品攻略查看","${ctx}/raiders/traiders/findById.action?cid="+obj, 1000,600);
}
         ...
         $(function(){       	 
         	...
         	remoteSort:false,
				...
				url: "${ctx}/raiders/traiders/findAllRaiders.action?isdel="+isdel,        
				frozenColumns:[[
				]],
				...
				columns:[[
					...
					{field:'contentandurl',title:'内容',width:0.2,align:'center' ,sortable:true,
						formatter:function(value,row,index){
		                   	...
		                   	if(row.type==1)
		                   		return "<a onclick='look("+row.id+")'>攻略文本查看</a>";
		                   	if(row.type==3)
		                   		return "--";
		                   	if(row.type==4)
		                   		return "--";
