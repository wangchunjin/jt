  ...
  <head>
  	<script type="text/javascript">
	  	function add(){	  		
	        ...
	        createSimpleWindow("addWindow","新增","${ctx}/web/JT/SYSPZ/addPic.jsp?currFrameId="+currFrameId, 400,300);
	    }
	    ...
	    function update(){
			...
			createSimpleWindow("eidtWindow","修改配置","${ctx}/sysconf/tsysconf/findById.action?id="+rows[0].id+"&currFrameId="+currFrameId,400,300);
	  	...
	  	function del(){
			...
			parent.LeftFrame.TwoFrame.$.messager.confirm("提醒","确定要删除?",function(res){
	        	...
	        	if(res){
					...
					var res = IntoActionByUrl("${ctx}/sysconf/tsysconf/delete.action?id="+ids);
// 					alert(res[0].result);
  ...
  <table width="100%">
       <tr>
          <td width="*" align="right">
          	 <epmis:button id="NewRecord" imageCss="icon-add" value="增加" action="add()" datactrCode="syspz_add.edit" ></epmis:button>
<!--           	 <epmis:button id="DeleteRecord" imageCss="icon-remove" value="删除" action="del()" datactrCode=""  ></epmis:button> -->
          	 <epmis:button id="NewRecord" imageCss="icon-edit" value="修改" action="update()" datactrCode="syspz_update.edit" ></epmis:button>
