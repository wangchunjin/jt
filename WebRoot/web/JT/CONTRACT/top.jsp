  ...
  <head>
  	<script type="text/javascript">
	  	function add(){	  		
	        ...
	        createSimpleWindow("addWindow","新增","${ctx}/web/JT/CONTRACT/addPic.jsp?currFrameId="+currFrameId, 450,250);
	    }
	    ...
	    function update(){
			...
			createSimpleWindow("eidtWindow","修改配置","${ctx}/contract/tcontract/findById.action?id="+rows[0].lender_id+"&currFrameId="+currFrameId,450,250);
	    ...
	    function look(){
			...
			createSimpleWindow("eidtWindow","出借人详情","${ctx}/contract/tcontract/findById.action?id="+rows[0].lender_id+"&see=1"+"&currFrameId="+currFrameId,450,250);
	  	...
	  	function del(){
			...
			parent.LeftFrame.TwoFrame.$.messager.confirm("提醒","确定要删除?",function(res){
	        	...
	        	if(res){
					...
					var res = IntoActionByUrl("${ctx}/contract/tcontract/delete.action?id="+ids);
// 					alert(res[0].result);
