 ...
 <script type="text/javascript">
			 function  AddRecord(){ 
			    ...
			    var res = $("#addForm").SubmitForm("${ctx}/sys/struct/addLanguage.action");
			     if(res[0].result=="success"){
			         var node = opener.parent.TwoFrame.$('#LanguageTree').treegrid('getSelected');
			         if(isNull(node)){                  
			        	 opener.parent.TwoFrame.TreeGrid_RefreshNode(node.ID,"LanguageTree");                   
			          }else{
			        	  opener.parent.TwoFrame.location.reload();   
			          }		 
			         window.close();
  ...
  <body  class="NewWinClass" onload="init()">
   <form method="post" action="" id="addForm" name="addForm">
        ...
        <input type="hidden" id="structLanguage.isPublic" name="structLanguage.isPublic" value="N">	
        <input type="hidden" id="structLanguage.createdBy" name="structLanguage.createdBy" value="${sessionScope.UserInfo.userId}">					
			...
			<div class="BottomDiv">
				<a href="###" class="btn_01" onclick="AddRecord();">确定<b></b></a>
					<a href="###" class="btn_01" onclick=" window.close();">关闭<b></b></a>				
			</div>
  </body>
</html>
