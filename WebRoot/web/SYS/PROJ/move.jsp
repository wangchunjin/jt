...
<html>
   ...
   function  AddRecord(){
		   ...
		   var node = GetIndexFrame(currFrameId).TwoFrame.$('#ProjTree').treegrid('getSelected');
	       if(isNull(node)){                  
		 	   orginId = node.PROJ_ID;                
	       }else{
	      ...
	      var parentNode = $('#EpsTree').treegrid('getSelected');
		   if(isNull(parentNode)){
			   parentId = parentNode.id;
			    ...
			    $.messager.alert("错误","请选择要EPS节点");
	    	    return;
		   }
           var res =  IntoActionByUrl("${ctx}/sys/proj/MoveProj.action?orginId="+orginId+"&parentId="+parentId)
          if(res[0].result=="success"){
        	 GetIndexFrame(currFrameId).TwoFrame.$('#ProjTree').treegrid('remove', orginId);  
             GetIndexFrame(currFrameId).TwoFrame.$('#ProjTree').treegrid('reloadFooter');
        	  if(parentId=="0"){
        		  GetIndexFrame(currFrameId).TwoFrame.location.reload();
        	  }else{
        		GetIndexFrame(currFrameId).TwoFrame.TreeGrid_RefreshNode(parentId,"ProjTree");  
        	  }
