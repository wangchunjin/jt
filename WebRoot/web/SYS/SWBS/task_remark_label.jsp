...
<html>
  ...
  function SaveRecord(){
     var node = parent.TwoFrame.$('#SwbsTree').treegrid('getSelected');
	  ...
	  $("#smSwbs\\.remark").val(content)

                var ff = document.labelForm;
               
                ff.action="UpdateTask.action?type=taskremark";
			   	ff.submit();
  }
 	  	function MCEInit()
	{
		var content=$("#smSwbs\\.remark").val();
   ...
   <body  onload="MCEInit()">
     ...
     <form method="post" action="" id="labelForm" name="labelForm">
       <input type="hidden" id="smSwbs.swbsId" name="smSwbs.swbsId" value="${smSwbs.swbsId}">	
         <input type="hidden" id="smSwbs.remark" name="smSwbs.remark" value="${smSwbs.remark}">							
       </form>
     </div> 						
  </body>
</html>
