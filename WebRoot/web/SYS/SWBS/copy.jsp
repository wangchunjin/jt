...
<html>
	<head>
        ...
        $(function(){ 
        	$('#SwbsTree').treegrid({
				...
				url: '${ctx}/sys/swbs/ShowSwbsMoveTree.action?parentId=0&swbs_type_id=${param.swbs_type_id}&level=2',
				idField:'SWBS_ID',
				...
				columns:[[
	 			...
	 			onBeforeExpand:function(node){
					$('#SwbsTree').treegrid('options').url = "${ctx}/sys/swbs/ShowSwbsMoveTree.action?swbs_type_id=${param.swbs_type_id}&parentId=" + node.SWBS_ID;
				} 
			});
		  	});
 ...
 function  AddRecord(){
	      ...
	      var parentNode = $('#SwbsTree').treegrid('getSelected');
           ...
           var res =  IntoActionByUrl("${ctx}/sys/swbs/CopySwbs.action?orginId="+orginId+"&parentId="+parentId+"&swbs_type_id=${param.swbs_type_id}")
