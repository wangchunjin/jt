			 ...
			 var userId = '${param.userId}';
	        	...
	        	var res = IntoActionByUrl("${ctx}/sys/user/AddRoleUser.action?userId="+userId+"&datas="+datas);
		    		if(res[0].result=="success"){
		    			var node = GetIndexFrame(currFrameId).TwoFrame.$('#SysUserTable').datagrid('getSelected');
		    			var index = GetIndexFrame(currFrameId).TwoFrame.$("#SysUserTable").datagrid('getRowIndex',node);
						GetIndexFrame(currFrameId).TwoFrame.$('#SysUserTable').datagrid('updateRow',{
							index: index,
							row: {
