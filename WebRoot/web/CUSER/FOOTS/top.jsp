  ...
  <head>
  	<script type="text/javascript">
	  	function add(){
	        ...
	        createSimpleWindow("addWindow","新增","${ctx}/web/CUSER/FOOTS/addPic.jsp?currFrameId="+currFrameId+"&uid="+uid, 600,400);
	    }
		function ck(){
	     	...
	     	var currFrameId =  parent.frameElement.id; 	     	
	     	if(rows.length==0){
	     		createSimpleWindow("addWindow","查看","${ctx}/web/CUSER/FOOTS/GROUPLISTF/FS.jsp?currFrameId="+currFrameId, 1000,550);
			}else{
				createSimpleWindow("addWindow","查看","${ctx}/web/CUSER/FOOTS/GROUPLISTF/FS.jsp?currFrameId="+currFrameId+"&uid="+rows[0].uid, 1000,500);
			}
	    ...
	    function update(){
			...
			createSimpleWindow("eidtWindow","修改配置","${ctx}/foots/tfoots/findById.action?id="+rows[0].id+"&currFrameId="+currFrameId,300,150);
	  	...
	  	function del(){
			...
			parent.LeftFrame.TwoFrame.$.messager.confirm("提醒","确定要删除?",function(res){
	        	...
	        	if(res){
					var res = IntoActionByUrl("${ctx}/foots/tfoots/delete.action?id="+ids);
					//alert(res[0].result);
