  ...
  <head>
  	<script type="text/javascript">
	  	function add(){
	        ...
	        createSimpleWindow("addWindow","新增","${ctx}/web/CUSER/JPUSER/addBrand.jsp?currFrameId="+currFrameId, 600,400);
	    }
...
// 		}
		...
		function isrz(){
	    	...
	    	if(rows.length==0){
				parent.LeftFrame.TwoFrame.$.messager.alert("错误","请选中要认证的金牌顾问！");
				return;
			}
	    	if(rows[0].isrenzhen!=1){
				parent.LeftFrame.TwoFrame.$.messager.alert("错误","非申请认证状态不能认证！");
				return;
			}
			createSimpleWindow("eidtWindow","银行卡号认证","${ctx}/user/tuser/findByIdrz.action?id="+rows[0].id+"&currFrameId="+currFrameId,600,500);
...
<%-- <epmis:button id="NewRecord" imageCss="icon-edit" value="推广" action="tg()" datactrCode="tuser_jptg.edit" ></epmis:button> --%>
			 <epmis:button id="NewRecord" imageCss="icon-add" value="认证" action="isrz()" datactrCode="jprenzhen_rz.edit" ></epmis:button>
          	 <epmis:button id="NewRecord" imageCss="icon-edit" value="查看" action="update()" datactrCode="jprenzhen_update.edit" ></epmis:button>
