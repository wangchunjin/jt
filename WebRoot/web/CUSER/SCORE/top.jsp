  ...
  <head>
  	<script type="text/javascript">
	  	function add(){
	        ...
	        createSimpleWindow("addWindow","新增","${ctx}/web/CUSER/SCORE/addPic.jsp?currFrameId="+currFrameId+"&uid="+uid, 600,400);
	    }
	    function update(){
			...
			createSimpleWindow("eidtWindow","修改配置","${ctx}/score/tscore/findById.action?id="+rows[0].id+"&currFrameId="+currFrameId,300,150);
	  	...
	  	function del(){
			...
			parent.LeftFrame.TwoFrame.$.messager.confirm("提醒","确定要删除?",function(res){
	        	...
	        	if(res){
					var res = IntoActionByUrl("${ctx}/score/tscore/delete.action?id="+ids);
					//alert(res[0].result);
...
<!--           	 <epmis:button id="NewRecord" imageCss="icon-add" value="增加" action="add()" datactrCode="" ></epmis:button> -->
          	 <epmis:button id="DeleteRecord" imageCss="icon-remove" value="删除" action="del()" datactrCode="score_delete.edit"  ></epmis:button>
